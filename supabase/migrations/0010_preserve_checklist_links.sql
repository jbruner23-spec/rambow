-- 0009's re-import preserved uploaded photos but not rmb_cards.parallel_id, so
-- every re-import silently wiped checklist links on all previously-seeded rainbows
-- (delete+insert in rmb_seed_inventory doesn't touch parallel_id). Extend the same
-- snapshot/restore to carry parallel_id alongside image_url.
...
begin
  ...
  select cs.player, cs.year, cs.product, cs.card_no, c.parallel_name, c.serial_no,
         c.image_url, c.parallel_id
  from rmb_cards c join rmb_card_sets cs on cs.id = c.card_set_id
  where (c.image_source = 'upload' and c.image_url is not null) or c.parallel_id is not null;
