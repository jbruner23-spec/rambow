-- The re-import snapshot only carried image_source='upload' rows, so every
-- spreadsheet sync wiped every eBay-sourced card image and the whole grid went
-- back to name placeholders until the weekly source_images refresh ran.
-- Preserve image_url/image_source for ALL sources, not just uploads.
--
-- Stale eBay URLs are safe to carry: CardArt falls back to the placeholder on
-- load error, and source_images.py refreshes them on its own schedule.
...
begin
  ...
  select cs.player, cs.year, cs.product, cs.card_no, c.parallel_name, c.serial_no,
         c.image_url, c.image_source, c.parallel_id, c.likely_your_copy
  from rmb_cards c join rmb_card_sets cs on cs.id = c.card_set_id
  where c.image_url is not null or c.parallel_id is not null;
  ...
  update rmb_cards c
     set image_url = u.image_url,
         image_source = u.image_source,
         likely_your_copy = coalesce(u.likely_your_copy, false)
  ...
  update rmb_cards c set parallel_id = u.parallel_id
  from _up u join rmb_card_sets cs on cs.player=u.player and cs.year=u.year
    and cs.product=u.product and cs.card_no=u.card_no
  where c.card_set_id = cs.id
    and c.parallel_name is not distinct from u.parallel_name
    and c.serial_no is not distinct from u.serial_no
    and u.parallel_id is not null;
